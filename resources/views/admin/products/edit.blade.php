            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                ...
                <div class="md:col-span-2">
                    ...
                    <div id="variants-container" class="space-y-2 mb-3">
                        @php $existingVariants = old('variant_labels') ? collect(old('variant_labels'))->zip(old('variant_prices', []))->all() : ($product->variants ?? []) @endphp
                        @foreach($existingVariants as $variant)
                            @php
                                $vLabel = is_array($variant) && isset($variant[0]) ? $variant[0] : ($variant['label'] ?? '');
                                $vPrice = is_array($variant) && isset($variant[1]) ? $variant[1] : ($variant['price'] ?? '');
                            @endphp
                        <div class="flex gap-2 items-center">
                            <input type="text" name="variant_labels[]" value="{{ $vLabel }}" placeholder="Label (e.g. 4GB / 64GB)"
                            ...
                            <input type="number" name="variant_prices[]" value="{{ $vPrice }}" placeholder="Price (KES)" min="0"
