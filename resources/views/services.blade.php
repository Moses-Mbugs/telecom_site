...
====================================================================== --}}
<div id="service-enquiry-modal" class="hidden fixed inset-0 z-[200000] flex items-center justify-center px-4">
    {{-- Backdrop --}}
    <div class="absolute inset-0 bg-black/60 backdrop-blur-sm"
        onclick="document.getElementById('service-enquiry-modal').classList.add('hidden')"></div>

    {{-- Modal card --}}
    <div class="relative bg-white rounded-3xl shadow-2xl w-full max-w-lg z-10 overflow-hidden max-h-[92vh] flex flex-col">

        {{-- Header --}}
        <div class="bg-gradient-to-r from-[#1e3040] to-[#2d4a60] px-8 py-6 flex items-center justify-between flex-shrink-0">
            <div>
                <div class="flex items-center gap-2 mb-1">
                    <div class="w-7 h-7 bg-[#b5342a] rounded-lg flex items-center justify-center flex-shrink-0">
                    ...
                    <h3 class="text-white font-bold text-lg">Service Enquiry</h3>
                </div>
            ...
            <button onclick="document.getElementById('service-enquiry-modal').classList.add('hidden')"
                class="w-9 h-9 bg-white/10 hover:bg-white/20 rounded-full flex items-center justify-center text-white transition flex-shrink-0">
        ...
        <div class="px-8 py-10 text-center flex-1 flex flex-col items-center justify-center">
            <div class="w-16 h-16 bg-green-100 rounded-full flex items-center justify-center mx-auto mb-4">
                <svg class="w-8 h-8 text-green-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"></path></svg>
            </div>
            <h4 class="text-xl font-bold text-gray-900 mb-2">Enquiry Submitted!</h4>
            <p class="text-gray-500 text-sm mb-6">{{ session('service_enquiry_success') }}</p>
            <button onclick="document.getElementById('service-enquiry-modal').classList.add('hidden')"
                class="px-8 py-3 bg-[#1e3040] text-white font-bold rounded-xl hover:bg-[#b5342a] transition-colors">
                Close
            </button>
        </div>
        @else
            ...
            <form action="{{ route('service-enquiry.store') }}" method="POST" class="px-8 py-6 space-y-4">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="text" name="name" value="{{ old('name') }}" required placeholder="John Doe"
                            class="w-full border border-gray-300 rounded-xl px-4 py-2.5 text-sm focus:ring-2 focus:ring-[#b5342a]/30 focus:border-[#b5342a] outline-none transition @error('name') border-red-400 @enderror">
                        @error('name')<p class="text-xs text-red-500 mt-1">{{ $message }}</p>@enderror
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-600 mb-1">Phone Number</label>
                        <input type="tel" name="phone" value="{{ old('phone') }}" placeholder="+254 7XX XXX XXX"
                            class="w-full border border-gray-300 rounded-xl px-4 py-2.5 text-sm focus:ring-2 focus:ring-[#b5342a]/30 focus:border-[#b5342a] outline-none transition">
                    </div>
                </div>
                ...
                <div>
                    ...
                    <input type="email" name="email" value="{{ old('email') }}" required placeholder="you@example.com"
                        class="w-full border border-gray-300 rounded-xl px-4 py-2.5 text-sm focus:ring-2 focus:ring-[#b5342a]/30 focus:border-[#b5342a] outline-none transition @error('email') border-red-400 @enderror">
                    <p class="text-xs text-gray-400 mt-1">A confirmation email will be sent here.</p>
                    @error('email')<p class="text-xs text-red-500 mt-1">{{ $message }}</p>@enderror
                </div>

                <div>
                    <label class="block text-sm font-medium text-gray-600 mb-1">Service of Interest</label>
                    <select name="service_interest"
                        class="w-full border border-gray-300 rounded-xl px-4 py-2.5 text-sm bg-white focus:ring-2 focus:ring-[#b5342a]/30 focus:border-[#b5342a] outline-none transition">
                ...
                <div>
                    <label class="block text-sm font-medium text-gray-600 mb-1">Your Message <span class="text-red-500">*</span></label>
                    <textarea name="message" rows="4" required placeholder="Tell us what you need..."
                        class="w-full border border-gray-300 rounded-xl px-4 py-2.5 text-sm focus:ring-2 focus:ring-[#b5342a]/30 focus:border-[#b5342a] outline-none transition resize-none @error('message') border-red-400 @enderror">{{ old('message') }}</textarea>
                    @error('message')<p class="text-xs text-red-500 mt-1">{{ $message }}</p>@enderror
                </div>

                <button type="submit"
                    class="w-full py-3 bg-[#b5342a] hover:bg-[#9c2820] text-white font-bold rounded-xl transition-all duration-300 hover:scale-[1.02] shadow-lg shadow-red-500/20">
                    Submit Enquiry
                </button>
            </form>
        </div>
